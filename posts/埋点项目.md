...
| 维度         | 数据监控（用户行为）                                         | 性能监控（页面性能）                                         | 异常监控（产品 & 系统异常）                                  |
| :----------- | :----------------------------------------------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| **核心目标** | 还原用户旅程，驱动产品决策                                   | 发现瓶颈，优化体验，提升业务指标                             | 快速定位、降低损失、保证稳定性                               |
| **关键指标** | PV、UV、停留时长、点击热力、转化漏斗、事件触发次数（addCart、share、pay） | FCP、LCP、FID、CLS、TTFB、首包时间、JS加载耗时、API耗时、静态资源404/5xx | JS Error（message+stack）、Promise Rejection、资源加载失败、API 4xx/5xx、业务逻辑错误、白屏检测 |
| **采集方式** | 1. 手动代码埋点（track） 2. 全埋点（自动抓取 dom 事件） 3. 可视化埋点（圈选） | 1. PerformanceObserver / performance.getEntriesByType 2. Resource Timing、Navigation Timing、User Timing | 1. window.onerror / unhandledrejection 2. try-catch + 手动 captureException 3. 资源标签 onerror 4. 心跳检测 + 白屏采样 |
| **上报时机** | 1. 实时上报（immediate） 2. 延迟合并（delay + batch） 3. 退出 flush | 1. 页面 onload 后统一收集 2. 超过阈值立即告警（如 LCP>2.5 s） | 1. 立即上报（error 队列） 2. 失败缓存到 localStorage，下次进入重试 |
